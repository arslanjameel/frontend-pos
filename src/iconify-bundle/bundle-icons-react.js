...
;(async function () {
  ...
  if (sources.json) {
    for (let i = 0; i < sources.json.length; i++) {
      ...
      let content = JSON.parse(
        await fs_1.promises.readFile(filename, 'utf8'),
      )

      // Filter icons
      if (typeof item !== 'string' && item.icons?.length) {
        const filteredContent = (0, utils_1.getIcons)(
          content,
          item.icons,
        )
        if (!filteredContent) {
      ...
      for (const key in content) {
        if (
          key === 'prefix' &&
          content.prefix === 'tabler'
        ) {
          for (const key in content.icons) {
            content.icons[key].body = content.icons[
              key
            ].body.replace(
              /stroke-width="2"/g,
              'stroke-width="1.5"',
      ...
      bundle +=
        'addCollection(' + JSON.stringify(content) + ');\n'
  ...
  if (sources.svg) {
    for (let i = 0; i < sources.svg.length; i++) {
      ...
      // Import icons
      const iconSet = await (0, tools_1.importDirectory)(
        source.dir,
        {
      ...
      console.log(
        `Bundled ${iconSet.count()} icons from ${
          source.dir
        }`,
      )
