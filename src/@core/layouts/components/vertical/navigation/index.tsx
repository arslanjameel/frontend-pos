...
const Navigation = (props: Props) => {
  ...
  const [currentActiveGroup, setCurrentActiveGroup] =
    useState<string[]>([])

  // ** Ref
  const shadowRef = useRef<{classList: any}>(null)
  ...
  const scrollMenu = (container: any) => {
    if (
      ...
      if (shadowRef && container.scrollTop > 0) {
        // @ts-ignore
        if (
          shadowRef.current && !shadowRef.current.classList.contains('scrolled')
        ) {
          // @ts-ignore
